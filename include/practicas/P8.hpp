...
vector<arista<double>> Tombuctu2(const vector<Ciudad>& ciudades,
  ...
  for (size_t v = 0; v < n; ++v) {
    ...
    for (size_t w = 0; w < n; ++w) {
      ...
      G[v][w] = ciudades[v].distancia(ciudades[w]);
    }
  }
  repre_1 = P.encontrar(0);
  ...
  for (size_t i = 1; i < n; ++i) {
    if (repre_1 == P.encontrar(i))
      islas[n_islas].push_back(i);
    else {
      ++n_islas;
      repre_1 = P.encontrar(i);
  ...
  for (size_t i = 0; i < n_islas; ++i) {
    for (size_t j = islas[i][0]; j <= islas[i][islas[i].size() - 1];
         ++j, costes_min = GrafoP<double>::INFINITO) {
      coste_act = std::accumulate(floyd[j].begin(), floyd[j].end(), 0,
                                  SumNotInf<double>());
